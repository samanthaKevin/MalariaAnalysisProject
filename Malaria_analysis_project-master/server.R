...
#library(gridExtra)
#library(XML)
boundaries<-readOGR(dsn="Data/statistical-gis-boundary-files", layer="District_Boundaries_2014")
...
function(input, output, session) {
 ...
 # })
  ...
  output$box1 <- renderPlot({
    ...
    box_plot_fetch1 <- read.csv("C:/Users/Samantha/Documents/ByAge.csv", header = TRUE)
  ...
  output$box2 <- renderPlot({
    #box_plot_fetch <- fetch(dbSendQuery(malariadb, paste("SELECT * FROM district")))
    #distinctAge = box_plot_fetch %>% distinct(Age_group)
    box_plot_fetch2 <- read.csv("C:/Users/Samantha/Documents/ByAge.csv", header = TRUE)
